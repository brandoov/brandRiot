/**
 * Mocks portados de _design/Scripts/data.js. Servem como fallback enquanto
 * o backend ainda não fornece dados agregados (progressão de LP, top 5 do
 * mês, tier list, heatmaps). Cada card que consome um mock renderiza
 * <MockBadge/> para deixar claro que aquele dado é simulado.
 */
...
export interface ChampPoolItem extends PoolItemBase {
  wr: number;
  kda: number;
}
export interface AgentPoolItem extends PoolItemBase {
  wr: number;
  kda: number;
}
export interface CompPoolItem extends PoolItemBase {
  top4: number;
  avg: number;
}
export interface DeckPoolItem extends PoolItemBase {
  wr: number;
  archetype: string;
}

export type PoolItem = ChampPoolItem | AgentPoolItem | CompPoolItem | DeckPoolItem;
...
export type AnyMatchMock = LolMatchMock | ValMatchMock | TftMatchMock | LorMatchMock;

export interface GameDataMock {
  summoner: { name: string; tag: string; level: number; region: string };
  rank: { tier: string; division: string; lp: number; wins: number; losses: number; top4?: number };
  rankProgression: number[];
  winRate: number[];
  matches: AnyMatchMock[];
  pool: PoolItem[];
  metaPicks: MetaPick[];
}

export const MOCK_DATA: Record<GameKey, GameDataMock> = {
  lol: {
...
export const GAME_META: Record<GameKey, { name: string; short: string; tagline: string; region: string; accent: string }> = {
...
export function makeHeatmap(seed = 1): number[][] {
  const m: number[][] = Array.from({ length: 10 }, () => Array(10).fill(0));
  const zones = [
    { x: 5, y: 5, r: 2.5, w: 0.95 },
    { x: 4, y: 4, r: 1.8, w: 0.7 },
    { x: 6, y: 6, r: 1.8, w: 0.7 },
    { x: 3, y: 6, r: 1.5, w: 0.5 },
    { x: 7, y: 3, r: 1.5, w: 0.5 },
